...
from makeBehaviorList import make_behavior_list
from moveToIndex import move_to_index
from markIndex import mark_index
...
class MainGUI(QMainWindow):
    ...
    def create_toolbar(self) -> None:
        ...
        self.left_side_toolbar.addWidget(self.next_behavior_index_button)
        self.left_side_toolbar.addWidget(self.behavior_index_completion)
        self.left_side_toolbar.addWidget(self.mark_behavior_index_button)

        self.right_side_toolbar = QToolBar('Sequence Toolbar')
        self.addToolBar(Qt.RightToolBarArea, self.right_side_toolbar)
        self.right_side_toolbar.addWidget(QtWidgets.QLabel('Mark Frame Sequence'))
        ...
        self.right_side_toolbar.addWidget(QtWidgets.QLabel('Behaviors'))
    ...
    def create_widgets(self) -> None:
        ...
        self.body_parts_list.addItems(self.behaviors)
        ...
        self.scrollArea.setFixedHeight(210)

        self.done_label_button = QtWidgets.QPushButton('Mark Behavior')
        ...
        self.next_behavior_index_button = QtWidgets.QPushButton('Next B Index')
        self.next_behavior_index_button.setFont(font)
        self.next_behavior_index_button.setFixedWidth(100)
        self.next_behavior_index_button.clicked.connect(self.event_move_to_index)
        self.next_behavior_index_button.setShortcut(QKeySequence("z"))

        self.mark_behavior_index_button = QtWidgets.QPushButton('Mark B Index')
        self.mark_behavior_index_button.setFont(font)
        self.mark_behavior_index_button.setFixedWidth(100)
        self.mark_behavior_index_button.clicked.connect(self.event_mark_indexes)
        self.mark_behavior_index_button.setShortcut(QKeySequence("Ctrl+m"))
    ...
    def event_done_labeling(self) -> None:
        try:
            individual = self.label_animal.currentText()
            behavior = self.body_parts_list.currentRow()
            behavior = self.behaviors[behavior]
            from_frame_number = int(self.frame_from.text())
            to_frame_number = int(self.frame_to.text())
            make_behavior_list(self.video_name, from_frame_number,
                               to_frame_number, individual, behavior)

        except AttributeError:
            QtWidgets.QMessageBox.warning(self, 'Error', 'Load the Video first')
        except TypeError:
            QtWidgets.QMessageBox.warning(self, 'Error', 'Provide the start and end point to mark behavior')
        except KeyError:
            QtWidgets.QMessageBox.warning(self, 'Error', 'Provide the start and end point to mark behavior')
        except ValueError:
            QtWidgets.QMessageBox.warning(self, 'Error', 'Provide the start and end point to mark behavior')
    ...
    def event_move_to_index(self) -> None:
        try:
            ...
            behavior = self.body_parts_list.currentRow()
            behavior = self.behaviors[behavior]
            self.goto_index, self.index_completion = move_to_index(self.video_name, individual,
                                                                   behavior, self.frame_number)
            if self.goto_index == '':
                self.goto_index = '0'
            try:
    ...
    def event_mark_indexes(self) -> None:
        try:
            individual = self.label_animal.currentText()
            behavior = self.body_parts_list.currentRow()
            behavior = self.behaviors[behavior]
            from_frame_number = int(self.frame_from.text())
            to_frame_number = int(self.frame_to.text())
            mark_index(self.video_name, individual, behavior,
                       from_frame_number, to_frame_number)
